...
def list_cars(request):
    if request.method == "POST" :
        ...
        car_dets = Listed_Cars(email = email , list_car_name = list_car_name , list_price = list_price , list_km_driven = list_km_driven , list_owners_count = list_owners_count , list_year = list_year , list_fuel_type = list_fuel_type , list_transmission_type = list_transmission_type , list_phone_number = list_phone_number , list_seller_name = list_seller_name , list_city = list_city , list_number_plate = list_number_plate )
...
def handleSignup(request):
    if request.method=="POST":
        ...
        myuser = User.objects.create_user(signup_email,signup_email,signup_password)
...
def dashboard(request):
    all_cars = Listed_Cars.objects.filter(email=request.user.username).order_by('list_price')
    if request.method=="POST":
        context = {
            "all_listed_cars" : all_cars,
            "message" : "Deleted",
